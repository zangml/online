                ...
                <div class="algo_input_container">
                                ...
                                <c:forEach items="${vo.paramList}" var="featureParam">
                                    <div class="input-group">
                                                <span class="input-group-addon"
                                                      title="${featureParam.des}">${featureParam.name}</span>
                                        <input type="text" class="form-control"
                                               name="${featureParam.shell}"
                                               value="${featureParam.defaultValue}">
                                    </div>
                                </c:forEach>
                            </form>
                            <button data-toggle="modal" data-target="#ajaxloader2" data-backdrop="static"
                                    onclick="submitPre(${vo.feature.id})" class="btn btn-primary pull-right btn1">提交
                            </button>
                        </div>
                    </c:if>
                        ...
                        <div class="panel-body">
                            <form id="classifier${classifier.id}">
                                <c:forEach items="${classifier.params}" var="cparam">
                                    <div class="input-group">
                                                <span class="input-group-addon"
                                                      title="${cparam.paramDes}">${cparam.paramDes}</span>
                                        <input type="text" class="form-control" placeholder=""
                                               name="${cparam.paramName}"
                                               value="${cparam.defaultValue}">
                                        <input type="hidden" value="${classifier.path}" name="classifier"/>
                                    </div>
                                </c:forEach>
                            </form>
                            <button onclick="submitClassify(${classifier.id})"
                                    class="btn btn-primary pull- btn1">训练数据
                            </button>
                        </div>
                    </c:if>
                    <c:if test="${algorithm.type==2}">
                        ...
                        <div class="panel-body">
                            ...
                            <button data-toggle="modal" data-target="#ajaxloader2" data-backdrop="static"
                                    onclick="submitFeature(${vo.feature.id})"
                                    class="btn btn-primary pull-right"
                                    style="position: absolute;bottom: 0px;right: 20px;">提取特征
                            </button>
                        </div>
                    </c:if>
                </div>
                <div class="algo_figure_container">
                    <c:if test="${algorithm.type==0}">
                        <c:if test="${algorithm.id != 6}">
                            <button data-toggle="modal" data-target="#ajaxloader2" data-backdrop="static"
                                    onclick="initFeature(${vo.feature.id})" class="btn-primary btn btn2">查看原始数据
                            </button>
                        </c:if>
...
<div class="nav_container">
    <ul>
        ...
        <li><a href="#algo_train">算法训练</a></li>
            ...
            success: function (data) {
                ...
                for (var i = 0; i < 4; i++) {
                    ...
                    for (var j = 0; j < attributeList.length / 4; j++) {
                        code += '<td>' + attributeList[index++] + '</td>'
                    }
                    code += '</TR>';
                }
                code += '</tbody></table>';
