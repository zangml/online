...
import com.koala.learn.service.VoteService;
...
//	@PreAuthorize("hasAnyAuthority('ROLE_ADMIN','ROLE_USER')")  // 指定角色权限才能操作方法
	public ResponseEntity<Response> createVote(Long blogId) {
		if(holder.getUser()==null){
			return ResponseEntity.ok().body(new Response(false, "未登录，不能点赞"));
		}
		try {
			System.out.println("创建点赞");
...
//	@PreAuthorize("hasAnyAuthority('ROLE_ADMIN','ROLE_USER')")  // 指定角色权限才能操作方法
	public ResponseEntity<Response> delete(@PathVariable("id") Integer id, Long blogId) {
		if(holder.getUser()==null){
			return ResponseEntity.ok().body(new Response(false, "未登录，不能取消点赞"));
		}
		boolean isOwner = false;
		Integer userId = voteService.getVoteById(id).getUserId();
